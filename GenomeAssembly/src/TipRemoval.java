...
public class TipRemoval {
    ...
    static int t = k + 1;           // a path length threshold of t. stop exploring if path length is greater than k+1.
    ...
    public static void bubbleHandler(Vertex[] graph) {
        for (int i = 0; i < graph.length; i++) {
            if (graph[i].removed || graph[i].outedges.size() < 2) {
                System.out.println("bubbleHandler");
                continue;
            }
    ...
    private static void explore(Vertex[] graph, Node node, HashSet<Integer> set) {
        ...
        //stop exploring if path length is greater than t.
        if (set.size() >= t) {
...
//        k = 3;
//        t = 3;
        String[] reads = {"AACG", "AAGG", "ACGT", "CAAC", "CGTT", "GCAA", "GTTG", "TCCA", "TGCA", "TTGC"};
